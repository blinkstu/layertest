...
class AdminStaff2Controller extends AdminBaseController
{
    //首页表格
    ...
    public function staffFetchData(){
        ...
        $user = new UserModel(); 
        $count = $user::hasWhere('role',['role_id'=>'4'])->order("create_time DESC")->where('user_type=1')->select();
        $list = $user::hasWhere('role',['role_id'=>'4'])->order("create_time DESC")->where('user_type=1')->with('role.roleinfo,desc')->page($page.','.$limit)->select();
        $result = [
            'code' => 0,
            'msg'  => '',
            'count'=> count($count),
    ...
    public function addStaffPost(){
        if ($this->request->isPost()) {
            $validate = new Validate([
                ...
                'user_pass' => 'require',
                'mobile' => 'number|length:11|unique:user,mobile',
            ],[
            ...
            if ($result !== true) {
                ...
                if ($result !== false) {
                    //添加一个角色 3号 工作人员
                    Db::name('RoleUser')->insert(["role_id" => 4, "user_id" => $result]);
