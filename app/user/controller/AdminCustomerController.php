...
class AdminCustomerController extends AdminBaseController
{
    ...
    public function staffFetchData(){
        ...
        $user = new UserModel(); 
        $count = $user::hasWhere('role',['role_id'=>'5'])->order("create_time DESC")->where('user_type=1')->select();
        $list = $user::hasWhere('role',['role_id'=>'5'])->order("create_time DESC")->where('user_type=1')->with('role.roleinfo,desc')->page($page.','.$limit)->select();
        $result = [
            'code' => 0,
            'msg'  => '',
            'count'=> count($count),
    ...
    public function addStaffPost(){
        if ($this->request->isPost()) {
            ...
            if ($result !== true) {
                ...
                if ($result !== false) {
                    Db::name('RoleUser')->insert(["role_id" => 5, "user_id" => $result]);
